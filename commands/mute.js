...
module.exports.run = async (blaki, message, args) => {
  ...
  if(!mutetime) return message.reply("❌ **Nie podano czasu wyciszenia** ❌");
   ...
   try{
        ...
        .setFooter('Zostałeś wyciszony!', `${config.avatar}`);
  ...
  .addField("Powód", reason)
  .setTimestamp(message.createdAt)
  .setFooter('Wyciszono Użytkownika', `${config.avatar}`);
  ...
  message.channel.send(`Użytkownik <@${mute.id}> został wyciszony na ${mutetime}`)
